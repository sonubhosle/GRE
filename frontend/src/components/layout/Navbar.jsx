        ...
        <nav className={`sticky top-0 z-[100] transition-all duration-500 w-full ${scrolled
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between items-center h-20">
                    ...
                    <div className="hidden md:flex items-center gap-6">
                        <Link to="/courses" className="text-[10px] font-black uppercase tracking-widest text-slate-500 hover:text-indigo-600 transition-colors">Academy</Link>
                            ...
                            {isAuthenticated ? (
                                <>
                                    <Link to="/notifications" className="relative p-2.5 rounded-xl bg-slate-50 hover:bg-slate-100 transition-all border border-slate-200 group">
                                        ...
                                        {unreadCount > 0 && (
                                            <span className="absolute -top-1 -right-1 w-5 h-5 bg-indigo-600 text-[10px] font-black text-white rounded-full flex items-center justify-center border-2 border-white">
                                                {unreadCount}
                                            </span>
                                        )}
                                    </Link>
                                    ...
                                    <div className="relative">
                                        ...
                                        <div className={`absolute right-0 mt-4 w-64 py-3 bg-white rounded-2xl border border-slate-200 shadow-2xl transition-all duration-300 origin-top-right ${profileOpen ? 'opacity-100 scale-100 translate-y-0 pointer-events-auto' : 'opacity-0 scale-95 -translate-y-2 pointer-events-none'
                                            ...
                                            <div className="px-5 py-3 border-b border-slate-100 mb-2">
                                                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Authorized Profile</p>
                                                <p className="text-xs font-black text-slate-900 truncate">{user?.email}</p>
                                            </div>
                                            <Link to={user?.role === 'ADMIN' ? '/admin/dashboard' : user?.role === 'TEACHER' ? '/teacher/dashboard' : '/dashboard'} className="flex items-center gap-3 px-5 py-3 text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 transition-all text-[10px] font-black uppercase tracking-widest">
                                                <LayoutDashboard size={16} className="text-indigo-600" /> Dashboard Matrix
                                            </Link>
                                            <Link to="/profile" className="flex items-center gap-3 px-5 py-3 text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 transition-all text-[10px] font-black uppercase tracking-widest">
                                                <UserCircle size={16} className="text-indigo-600" /> Identity Matrix
                                            </Link>
                                            <div className="h-px bg-slate-100 mx-5 my-2"></div>
                                            <button onClick={handleLogout} className="w-full flex items-center gap-3 px-5 py-3 text-rose-600 hover:text-rose-500 hover:bg-rose-50 transition-all text-[10px] font-black uppercase tracking-widest">
                                ...
                                <div className="flex items-center gap-4">
                                    <Link to="/login" className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 hover:text-indigo-600 transition-colors">Login</Link>
                                    <Link to="/register" className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all shadow-lg shadow-indigo-500/20 active:scale-95">Enroll Now</Link>
                                </div>
                            )}
                        </div>
                    </div>
            ...
            <div className={`md:hidden transition-all duration-300 overflow-hidden ${isOpen ? 'max-h-[500px] border-t border-slate-100' : 'max-h-0'}`}>
                <div className="px-4 py-8 space-y-4 bg-white shadow-xl">
                    <Link to="/courses" className="block text-[10px] font-black uppercase tracking-widest text-slate-500 hover:text-indigo-600 transition-colors">Academy Archive</Link>
                    {!isAuthenticated && (
                        <div className="pt-4 flex flex-col gap-4">
                            <Link to="/login" className="text-center text-[10px] font-black uppercase tracking-widest text-slate-500">Login</Link>
                            <Link to="/register" className="bg-indigo-600 text-center text-white py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest">Enroll Now</Link>
                        </div>
                    )}
                </div>
            </div>
